...
function renderRetornosAdmin(rows = []) {
  const list = document.getElementById('inspetoresLista');
  const resumo = document.getElementById('inspetoresResumo');
  if (!list) return;

  if (!rows.length) {
    if (resumo) resumo.textContent = 'Nenhum retorno encontrado.';
    list.innerHTML = `<div style="padding:1rem;color:var(--medium-gray);text-align:center;">Sem dados.</div>`;
    return;
  }

  if (resumo) resumo.textContent = `Retornos: ${rows.length}`;
  ...
  rows.forEach(r => {
    ...
    const dia = ymdFromTimestamp(r.createdAt) || ymdFromTimestamp(r.updatedAt) || r.dataRef || '';
    div.innerHTML = `
      <div class="history-info">
        <h3 style="margin:0 0 4px 0;">${r.elemento || '—'} <span style="font-weight:500;color:var(--medium-gray)">(${r.incidencia || '—'})</span></h3>
        <p style="margin:0;"><b>Inspetor:</b> ${r.inspectorEmail || '—'} &nbsp; • &nbsp; <b>Regional:</b> ${r.regional || '—'} &nbsp; • &nbsp; <b>Dia:</b> ${dia || '—'}</p>
        <p style="margin:3px 0 0 0;"><b>Cliente:</b> ${r.clienteAfetado || '—'} &nbsp; • &nbsp; <b>Causa:</b> ${r.causa || '—'}</p>
        <p style="margin:6px 0 0 0;"><b>Retorno:</b> ${String(r.retornoTexto || '').replace(/\n/g,'<br>')}</p>
      </div>
    `;
...
async function loadRetornosModalAdmin() {
  const dia = document.getElementById('inspFiltroData')?.value || '';
  const reg = document.getElementById('inspFiltroRegional')?.value || 'TODOS';

  const res = await DataService.getRetornosAdmin();
  const all = (res?.success && Array.isArray(res.data)) ? res.data : [];

  const filtered = all.filter(r => {
    // filtra por regional
    if (reg && reg !== 'TODOS') {
      const rr = String(r.regional || '').toUpperCase();
      if (rr !== reg) return false;
    }

    // filtra por dia (createdAt/updatedAt)
    if (dia) {
      const d1 = ymdFromTimestamp(r.createdAt);
      const d2 = ymdFromTimestamp(r.updatedAt);
  ...
  renderRetornosAdmin(filtered);
  return filtered;
}

function exportRetornosXLSX(rows = []) {
  if (!window.XLSX) {
...
function setupModalInspetoresAdmin() {
  // define data padrão = hoje
  const inp = document.getElementById('inspFiltroData');
  if (inp && !inp.value) {
    const d = new Date();
    const pad = (n) => String(n).padStart(2,'0');
  ...
  document.getElementById('btnInspetores')?.addEventListener('click', async () => {
    // abre modal e carrega
    openModal('modalInspetores');
    await loadRetornosModalAdmin();
  });

  document.getElementById('btnAtualizarInspetores')?.addEventListener('click', async () => {
    await loadRetornosModalAdmin();
  });

  document.getElementById('btnExportarInspetores')?.addEventListener('click', async () => {
    const rows = await loadRetornosModalAdmin();
    exportRetornosXLSX(rows);
  });
}

// ✅ chamar depois do initEventListeners() no init()
const __oldInit = init;
function init() {
  __oldInit();
  setupModalInspetoresAdmin();
}